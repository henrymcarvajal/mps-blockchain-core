...
public class AccountWithdrawer {
    ...
    public OperationResult withdrawFromSellerAddress(Credentials credentials, Map<String, String> outputs) {
        return withdrawFromAddress(SELLER_ACCOUNT_TYPE, credentials, outputs);
    }
    
    public OperationResult withdrawFromBuyerAddress(Credentials credentials, Map<String, String> outputs) {
    ...
    private OperationResult withdrawFromAddress(String accountType, Credentials credentials,
        ...
        Map<String, String> getBalanceOutput = new HashMap<>();
        if (isAccountWithdrawable(accountType, credentials, getBalanceOutput) == OperationResult.SUCCESS) {
            try {
                ...
                BigInteger amountToCollect = new BigInteger(getBalanceOutput.get("amountToCollect"));
                
                TransactionReceipt transactionReceipt = Transfer.sendFunds(web3j, credentials,
                        credentialsProvider.getMainAddress(), new BigDecimal(amountToCollect), Unit.WEI).send();
            ...
            outputs.put(GenericOperationOutputs.ERROR_MESSAGE,
                    getBalanceOutput.get(GenericOperationOutputs.ERROR_MESSAGE));
            return OperationResult.ERROR;
        }
    }
    ...
    private OperationResult isAccountWithdrawable(String accountType, Credentials credentials,
        ...
        try {
            ...
            if (accountType.contentEquals(SELLER_ACCOUNT_TYPE)) {
                amountToCollect = moneyCollectionPolicyForSeller(ethGetBalance.getBalance());
            } else if (accountType.contentEquals(BUYER_ACCOUNT_TYPE)) {
                amountToCollect = moneyCollectionPolicyForBuyer(ethGetBalance.getBalance());
            }
            
            if (amountToCollect.compareTo(BigInteger.ZERO) <= 0) {
                outputs.put(GenericOperationOutputs.ERROR_MESSAGE,
                        "not enough funds to withdraw from " + credentials.getAddress());
                return OperationResult.ERROR;
            } else {
                outputs.put("amountToCollect", amountToCollect.toString());
                return OperationResult.SUCCESS;
            }
    ...
    /**
     * Different alternatives to calculate minimum amount to transfer
     * 
     * DEFAULT_TRANSFER_GAS.multiply(DefaultGasProvider.GAS_PRICE);
     * DEFAULT_TRANSFER_GAS.multiply(DefaultGasProvider.GAS_PRICE).multiply(BigInteger.TEN).divide(BigInteger.valueOf(4L));
     * DEFAULT_TRANSFER_GAS.multiply(BigInteger.valueOf(4_000_000_000L));
     * 
     * @param amount
     * @return
     */
    private BigInteger moneyCollectionPolicyForSeller(BigInteger amount) {
        BigInteger minimumAccountAmountforTransfer = DEFAULT_TRANSFER_GAS
                .multiply(BigInteger.valueOf(sellerCollectionsGasPrice));
        return amount.subtract(minimumAccountAmountforTransfer);
    }
    
    private BigInteger moneyCollectionPolicyForBuyer(BigInteger amount) {
        BigInteger minimumAccountAmountforTransfer = DEFAULT_TRANSFER_GAS
                .multiply(BigInteger.valueOf(buyerCollectionsGasPrice));
        return amount.subtract(minimumAccountAmountforTransfer);
    }
}
