...
public class CompraventaSendBuyerDeposit implements ContractOperation {
    ...
    public OperationResult execute(Map<String, String> outputs) {
        ...
        Optional<BlockchainAccount> blockchainAccountO = blockchainAccountRepositoryService
                .findById(buyerAccount.getBlockchainAccountId());
        if (blockchainAccountO.isEmpty()) {
            return OperationResult.ERROR;
        }
        
        BlockchainAccount buyerBlockchainAccount = blockchainAccountO.get();
        ...
        try {
            ...
            RemoteCall<TransactionReceipt> transaction = compraventa.enviarFondosComprador(
                    Convert.toWei(new BigDecimal(inputParameters.getContractFee()), Unit.ETHER).toBigInteger());
